...
func (t *Displayer) Functions() template.FuncMap {
	return template.FuncMap{
		"strJoin": func(s ...interface{}) string {
			var strArr []string
			for _, v := range s {
				strArr = append(strArr, util.Interface2String(v))
			}
			return strings.Join(strArr, "")
		},
		"replace": func(str interface{}, oldStr interface{}, newStr interface{}) string {
			//logger.Error(str,oldStr,newStr)
			return strings.Replace(util.Interface2String(str), util.Interface2String(oldStr), util.Interface2String(newStr), -1)
		},
		//任意值转字符串
